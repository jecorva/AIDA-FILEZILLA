...
<script>
    ...
    function selectSupervisor(value, row, index) {
        ...
        html = [
            '<select  data-select="ope" id="sltSup_'+ index +'" class="select2">',
                '<option value="0" selected>Seleccionar</option>',
                option,
            '</select>'
        ].join('');
    ...
    async function saveRow(index, labor_id, location_ids, rdt_id) {
        ...
        if( implemento != "0" && maquinaria != "0" && operario != "0" && fiscal != '0' ) {
            ...
            if( response.status == 201 ) {
                ...
                $("#sltSup_"+index).prop('disabled', false)
            }
